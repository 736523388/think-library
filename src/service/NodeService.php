...
class NodeService extends Service
{
    ...
    public function getCurrent($type = '')
    {
        ...
        $middle = '\\' . $this->nameTolower($this->request->controller());
        $suffix = ($type === 'controller') ? '' : ('\\' . $this->request->action());
    ...
    public function getMethods($force = false)
    {
        static $data = [];
        if (empty($force)) {
            $data = $this->cache->get('system_auth_node');
            if (is_array($data) && count($data) > 0) return $data;
        }
        $ignore = get_class_methods('\library\Controller');
        foreach ($this->scanDirectory(APP_PATH) as $file) {
        ...
        $this->cache->set('system_auth_node', $data);
        return $data;
    }
